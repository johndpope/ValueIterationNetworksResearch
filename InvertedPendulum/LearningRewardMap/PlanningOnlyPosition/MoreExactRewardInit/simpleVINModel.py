...
def VIN(state_input, state_dim, action_dim, config, weights=None):
    ...
    if (weights == None):
        ...
        #Intialization of the reward map
        if config.numstates % 2 == 0:
            X = tf.scalar_mul(2*np.pi / config.numstates, tf.concat([tf.range(0, config.numstates/2), tf.range(config.numstates/2 - 1, -1, -1)], axis=0))
        else:
            X = tf.scalar_mul(2*np.pi / config.numstates, tf.concat([tf.range(0,config.numstates/2 + 1), tf.range(config.numstates/2, 0, -1)], axis=0))
        X = -1 * tf.square(X)
