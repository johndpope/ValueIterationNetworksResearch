...
def VIN(state_input, state_dim, action_dim, config, weights=None):
    ...
    if (weights == None):
        #Reward Map
        reward_hidden1 = tf.Variable(np.random.randn(2, config.vel_hidden1) * 0.001, dtype=tf.float32)
    ...
    velocities = tf.stack([state_input_Transpose[2], state_input_Transpose[3]])
    ...
    return state_input, output, [reward_hidden1, reward_bias1, reward_output,
        reward_biasOut, w, w_fb, w_h1, bias1, w_h2, bias2, w_o, bias_o]
