...
def VIN(state_input, state_dim, action_dim, config, weights=None):
    ...
    if (weights == None):
        ...
        state_hidden1 = tf.Variable(np.random.randn(config.state_hidden1, 2), dtype=tf.float32)
        state_bias1 = tf.Variable(np.random.randn(config.state_hidden1, 1), dtype=tf.float32)
        state_output = tf.Variable(np.random.randn(1, config.state_hidden1), dtype=tf.float32)
        state_biasOut = tf.Variable(np.random.randn(1,1), dtype=tf.float32)
        ...
        state_hidden1 = weights[4]
        state_bias1 = weights[5]
        state_output = weights[6]
        state_biasOut = weights[7]
        w     = weights[8]
        w_fb  = weights[9]
        w_h1   = weights[10]
        bias1 = weights[11]
    ...
    # Calculate Position
    stateHidden1 = tf.nn.relu(tf.matmul(state_hidden1, tf.stack([state_input_Transpose[0], state_input_Transpose[1]])) + state_bias1)
    state = tf.cast(tf.floordiv(tf.nn.tanh(tf.matmul(state_output, stateHidden1) + state_biasOut)[0] + 1, 2.0/numstates), tf.int32)

    #Select the conv-net channels at the state position
    ins1 = tf.range(batch_size)
    idx_in = tf.transpose(tf.stack([ins1, state]), [1,0])
    ...
    return state_input, output, [reward_hidden1, reward_bias1, reward_output,
        reward_biasOut, state_hidden1, state_bias1, state_output, state_biasOut,
        w, w_fb, w_h1, bias1, w_o, bias_o]
