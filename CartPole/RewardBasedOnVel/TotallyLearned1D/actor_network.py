...
tf.app.flags.DEFINE_integer('k',50, 'Number of Value Iterations')
tf.app.flags.DEFINE_integer('width', 5, 'Size of conv filter (~state connectivity)')
tf.app.flags.DEFINE_integer('ch_i', 1, 'Channels in input layer')
tf.app.flags.DEFINE_integer('ch_q', 30, 'Channels in q layer (~actions)')
tf.app.flags.DEFINE_integer('vel_hidden1', 30, "Size of first hidden layer for velocity")
tf.app.flags.DEFINE_integer('state_hidden1', 30, "Size of first hidden layer for velocity")
...
class ActorNetwork:
    ...
    def get_settings():
        return "numstates; %d \n num value iterations; %d \n conv type; circular \n" \
            ...
            config.ch_q, config.hidden1, config.vel_hidden1,
